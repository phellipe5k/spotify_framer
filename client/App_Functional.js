...
const getNowPlaying = (input) => {
  spotifyApi.search(input, ['track'])
    .then((response) => {
      setLoading(false);
      setMusics(response.tracks.items)
    })
}
...
useEffect(() => {
  ...
  return (
    <div className="App">
      <Quadro info={music} color={color} svg={svg} />
      <Interact>
        <img src={logo}/>
        <div className="music-wrap">
          <div className="texts">
            <SearchInput>
              ...
              <input type="text" onChange={handleChange} value={input}placeholder="Digite a música" />
            </SearchInput>
            <ColorInput>
          ...
          <MusicList musics={musics} onClick={handleClick}/>
